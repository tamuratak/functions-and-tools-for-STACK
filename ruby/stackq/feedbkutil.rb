...
stackqsimp0(ex) := fullratsimp( radcan( factcomb( ex ) ) );
does_hold(ex) := is( stackqsimp0(lhs(ex)-rhs(ex)=0) or stackqsimp(lhs(ex)-rhs(ex)=0) or zeromatrixp(stackqsimp(lhs(ex)-rhs(ex))) or ratsimp(rectform(ex)) );
declare(n, integer);
EOS
  end
